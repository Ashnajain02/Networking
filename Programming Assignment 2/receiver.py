...
def start_receiver(connection_ID, loss_rate=0.0, corrupt_rate=0.0, max_delay=0.0):
    ...
    expectedSEQ = 0
    print("expectedSEQ: ", expectedSEQ)

    while True:
        message = clientSocket.recv(30).decode("utf-8")
        print("{}: received: {}".format(datetime.datetime.now(), message))
        ...
        #received non-corrupt message with correct seq
        if checksum_verifier(message) == True and messageSEQ == str(expectedSEQ):
            #send pos ACK
            print("perfect")
            #add payload to data string
            data += message[4:24]
            ACK = messageSEQ
            expectedSEQ = flipBit(expectedSEQ)

        #duplicate packet received
        elif checksum_verifier(message) == True and messageSEQ != str(expectedSEQ):
            print("Duplicate")
            ACK = messageSEQ
            #ACK = 2
            #reply = "DUPLICATE"
        ...
        #received corrupt message or incorrect sequence
        elif checksum_verifier(message) == False or messageSEQ != str(expectedSEQ):
            print("corrupt message")
            total_corrupted_pkt_recv += 1
            #send neg ACK
            try:
                ACK = flipBit(int(messageSEQ))
            except:
                ACK = flipBit(int(messageACK))
        reply = "  " + str(ACK) + "                      "
        reply_checksum = checksum(reply)
        reply = reply + str(reply_checksum)
    
        #send response to sender
        #time.sleep(0.1)
        clientSocket.sendall(reply.encode("utf-8"))
        print("{}: sending : {}".format(datetime.datetime.now(), reply))
        total_packet_sent += 1
